...
export default function ScenarioBuilder() {
  ...
  return (
    <div className="p-8 max-w-7xl">
      <h2 className="text-2xl font-bold text-slate-800 mb-6">Scenario Builder</h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            ...
            <div className={`mt-3 text-sm font-medium ${Math.abs(allocatedAcres - baselineParams.total_acres) < 0.5 ? "text-emerald-600" : "text-red-500"}`}>
          ...
          {/* Livestock */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Livestock</h3>
          ...
          {/* Prices */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Prices</h3>
            <ParamInput label="Milk" value={params.milk_price_per_liter ?? 0} onChange={(v) => set("milk_price_per_liter", v)} prefix="$" suffix="/L" step={0.01} min={0} />
            <ParamInput label="Broccoli" value={params.broccoli_price_per_ton ?? 0} onChange={(v) => set("broccoli_price_per_ton", v)} prefix="$" suffix="/ton" step={10} min={0} />
            <ParamInput label="Flower revenue" value={params.flower_annual_revenue ?? 0} onChange={(v) => set("flower_annual_revenue", v)} prefix="$" suffix="/yr" step={1000} min={0} />
          </div>

          {/* Production */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Production</h3>
          ...
          {/* Costs */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Costs</h3>
          ...
          {/* Weather */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-600 uppercase tracking-wide mb-3">Weather</h3>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            ...
            <div className="space-y-2 text-lg">
              ...
              <div className={`text-sm text-right ${delta >= 0 ? "text-emerald-500" : "text-red-500"}`}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            ...
            <div className="grid grid-cols-3 gap-3 text-center mb-4">
              <div className="bg-red-50 rounded-lg p-3">
                <p className="text-xs text-red-400 uppercase">Bear</p>
                <p className="text-lg font-bold text-red-600">{formatCurrencyFull(results.bear.profit)}</p>
              </div>
              <div className="bg-emerald-50 rounded-lg p-3">
                <p className="text-xs text-emerald-400 uppercase">Base</p>
                <p className="text-lg font-bold text-emerald-600">{formatCurrencyFull(results.base.profit)}</p>
              </div>
              <div className="bg-blue-50 rounded-lg p-3">
                <p className="text-xs text-blue-400 uppercase">Bull</p>
                <p className="text-lg font-bold text-blue-600">{formatCurrencyFull(results.bull.profit)}</p>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 text-sm border border-slate-200 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
              />
              <button
                onClick={handleSave}
                disabled={saving || !scenarioName.trim()}
                className="bg-emerald-600 text-white text-sm font-medium px-4 py-2 rounded-md hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {saving ? "Saving..." : "Save"}
              </button>
            </div>
